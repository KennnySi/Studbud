...
function addCard(id) {
    ...
    div.appendChild(h5);
    h5.setAttribute('data-placeholder', 'Add text here');
    h5.classList = ['p-3 editable']
    initDraggable(div)
    document.getElementById(id).appendChild(div)
}

// event for add new card to a list
document.getElementById('addTodo').addEventListener('click', () => {
    addCard('todo')
});
document.getElementById('addPrgs').addEventListener('click', () => {
    addCard('prgs')
});
document.getElementById('addTest').addEventListener('click', () => {
    addCard('test')
});
document.getElementById('addDone').addEventListener('click', () => {
    addCard('done')
});
...
document.getElementById('addCol').addEventListener('click', () => {
    ...
    //     <div id="done" class="drop-zone"></div>
    //     <div>
    //         <img width="40" id="addDone" src="icons/plus.svg" alt="">
    //     </div>
    // </div>
    let col = document.createElement('div');
    let colTitle = document.createElement('div');
    let dropZone = document.createElement('div');
    let addCol = document.createElement('img');
    dropZone.classList.add('drop-zone');
    dropZone.id = 'drop-zone' + (kanban.childElementCount + 1);
    dropZone.addEventListener('dragenter', dropZoneEnterHandler);
    dropZone.addEventListener('dragover', dropZoneOverHandler);
    dropZone.addEventListener('dragleave', dropZoneLeaveHandler);
    dropZone.addEventListener('drop', dropZoneDropHandler);
    ...
    colTitle.classList.add('editable')
    colTitle.contentEditable = true;
    colTitle.setAttribute('data-placeholder', 'Add title here');
    col.appendChild(colTitle);
    col.appendChild(dropZone);
    col.appendChild(addCol);
    col.id = 'col' + (kanban.childElementCount + 1);
    addCol.src = 'icons/plus.svg';
